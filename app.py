...
.rt .ok{background:#e8f5e9;color:#1b5e20;font-weight:700;border-radius:3px;
        padding:2px 7px;display:inline-block}
.rt .lo{background:#ffebee;color:#e53935;font-weight:700;border-radius:3px;
        padding:2px 7px;display:inline-block}
.rt .pa{background:#fff3e0;color:#e65100;font-weight:700;border-radius:3px;
        padding:2px 7px;display:inline-block}
...
with st.sidebar:
    ...
    if mode == "Full Reconciliation":
        ...
        p1_start=start_date; p1_end=end_date
        p2_start=start_date; p2_end=end_date
    else:
    ...
    if st.session_state.run_done:
        ...
        for key, label, fname in [
            ("dl_order_wise",   "⬇️ Revenue Recon (Order Wise)", f"FN_OrderWise_{start_date}_{end_date}.xlsx"),
            ("dl_discrepancy",  "⬇️ Discrepancy Sheet",          f"FN_Discrepancy_{start_date}_{end_date}.xlsx"),
            ("dl_comparison",   "⬇️ Comparison Sheet",           f"FN_Comparison_{p1_start}_{p2_end}.xlsx"),
        ]:
            ...
            if data:
                ...
                st.caption(f"{label} — generating…")

        # ZIP of all output files
        ...
        if out_files:
            ...
            zb.seek(0)
            st.download_button("⬇️ All files (ZIP)", data=zb.getvalue(),
                file_name=f"FN_Reconciliation_{start_date}_{end_date}.zip",
...
st.markdown(f"""<div class="fn-hdr">
  ...
  <div class="fn-period">{'Full Reconciliation' if mode=='Full Reconciliation' else 'Comparison Mode'}<br>
    <b>{period_str}</b></div>
</div>""", unsafe_allow_html=True)

tab_upload, tab_run = st.tabs(["📁 Upload Files","🚀 Run & Results"])
...
with tab_upload:
    ...
    if api_files:
        st.success(f"✅ {len(api_files)} file(s) uploaded")

    st.markdown('<div class="sec">Phase 1 — Bank Statements</div>', unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    with c1:
        st.markdown("**🔷 Bridgerpay**"); bp_files =st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="bp",label_visibility="collapsed")
        st.markdown("**🔷 Payprocc**");   pp_files =st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="pp",label_visibility="collapsed")
        st.markdown("**🟢 Coinsbuy** _(API only)_");   cb_files =st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="cb",label_visibility="collapsed")
    with c2:
        st.markdown("**🟢 ZEN**");        zen_files=st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="zen",label_visibility="collapsed")
        st.markdown("**🟢 Confirmo** _(matches API + Orchestrator)_");   cfm_files=st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="cfm",label_visibility="collapsed")
        st.markdown("**🟢 TC Pay**");     tcp_files=st.file_uploader("Upload",accept_multiple_files=True,type=["csv","xlsx","xls"],key="tcp",label_visibility="collapsed")

    st.markdown('<div class="sec">Phase 2 — PSP Statements</div>', unsafe_allow_html=True)
    with st.expander("📂 PSPs under Bridgerpay"):
        ...
        with b2:
            ...
            # Confirmo (BP) uses the Phase 1 Confirmo file automatically
    ...
    with st.expander("📂 PSPs under Payprocc"):
        ...
        with p1c: dloc_f  =st.file_uploader("DLocal",    type=["csv","xlsx","xls"],key="p2_dl")
        with p2c: skrill_f=st.file_uploader("Skrill",    type=["csv","xlsx","xls"],key="p2_sk")
        with p3c: pspp_f  =st.file_uploader("Paysafe PP",type=["csv","xlsx","xls"],key="p2_spp")
...
with tab_run:
    ...
    # Confirmo Phase 2: reuse Phase 1 Confirmo file — April Confirmo matches both API & Orchestrator
    ...
    if not api_files:
        st.info("👆 Upload the API file first.")
    else:
        ...
        with c3: (st.info if uploaded_psps else st.caption)(
            f"📂 PSPs: {len(uploaded_psps)}" if uploaded_psps else "No PSP files (optional)")
            ...
            try:
                ...
                if detect_dupes and tc:
                    ...
                    if not dup_rows.empty:
                        ...
                        st.warning(f"⚠️ {len(dup_rows):,} rows with duplicate TxIDs detected")
                ...
                # ── Pre-generate the 3 downloads now (while spinner is showing) ──
                upd(92,"Building Order Wise report…")
                try:
                    ...
                    st.session_state.dl_order_wise = None
                    st.warning(f"Order Wise: {_e}")

                upd(94,"Building Discrepancy report…")
                try:
                    ...
                    st.session_state.dl_discrepancy = None
                    st.warning(f"Discrepancy: {_e}")

                upd(96,"Building Comparison report…")
                try:
                    ...
                    st.session_state.dl_comparison = None
                    st.warning(f"Comparison: {_e}")
        ...
        if st.session_state.run_done and st.session_state.results:
            ...
            if dup_df is not None and not dup_df.empty:
                with st.expander(f"🔍 Duplicate TxIDs — {len(dup_df):,} unique IDs duplicated"):
            ...
            try:
                ...
                if s:
                    ...
                    def kpi(col,val,lbl,sub,clr):
                        col.markdown(
                            f'<div class="kpi-card"><div class="kpi-val" style="color:{clr}">'
            ...
            try:
                ...
                def _tbl(rows, hdrs):
                    ...
                    bb="".join(rows)
                    return f'<div style="overflow-x:auto"><table class="rt"><thead><tr>{hh}</tr></thead><tbody>{bb}</tbody></table></div>'
                ...
                rows1=[
                    f'<tr><td class="nm">{r["Bank Name"]}</td>'
                ...
                if results.get("phase2"):
                    ...
                    rows2=[
                        f'<tr><td class="nm">{r["PSP Name"]}</td>'
                ...
                st.warning(f"Summary tables: {_e}")

            # Comparison mode extra table
            if mode == "Comparison":
                st.markdown('<div class="sec">Period Comparison</div>', unsafe_allow_html=True)
                try:
                    from engine.report_order_wise import build_order_wise
                    data = build_order_wise(api_en, results, results.get("phase2",{}))
                    if data:
                        p1l = data["p1_label"]; p2l = data["p2_label"]
                        comp_rows = [r for r in data["rows"] if not r["_is_total"] and
                                     (r["P1_api_n"]+r["P2_api_n"])>0]
                        if comp_rows:
                            comp_data = []
                            for r in comp_rows:
                                comp_data.append({
                                    "Package": r["Package"], "Plan": r["Plan"],
                                    f"{p1l} API": r["P1_api_n"], f"{p1l} Orch": r["P1_orch_n"],
                                    f"{p1l} PSP": r["P1_psp_n"], f"{p1l} Revenue": f"${r['P1_api_rev']:,.0f}",
                                    f"{p2l} API": r["P2_api_n"], f"{p2l} Orch": r["P2_orch_n"],
                                    f"{p2l} PSP": r["P2_psp_n"], f"{p2l} Revenue": f"${r['P2_api_rev']:,.0f}",
                                    "Diff Orders": r["P2_api_n"]-r["P1_api_n"],
                                    "Diff Revenue": f"${r['P2_api_rev']-r['P1_api_rev']:,.0f}",
                                })
                            st.dataframe(pd.DataFrame(comp_data), use_container_width=True, hide_index=True)
                except Exception as _e:
                    st.warning(f"Comparison table: {_e}")
